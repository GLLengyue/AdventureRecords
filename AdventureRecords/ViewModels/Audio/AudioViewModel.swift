...
class AudioViewModel: NSObject, ObservableObject, AVAudioPlayerDelegate {
    ...
    private func createAudioDirectoryIfNeeded() {
        ...
        if !fileManager.fileExists(atPath: audioDirectoryPath.path) {
            do {
                ...
                debugPrint("创建音频目录: \(audioDirectoryPath.path)")
            } catch {
    ...
    func loadRecordings() {
        ...
        recordings = allRecordings.filter { recording in
            ...
            if !fileExists {
                debugPrint("警告: 录音文件不存在: \(recording.recordingURL.path)")
                // 可选：删除不存在文件的数据库记录
    ...
    func stopRecording(save: Bool) {
        ...
        if save, let recording = currentRecording {
            ...
            debugPrint("Recording stopped, ready to be saved: \(recording.title)")
        } else if !save, let recordingURL = currentRecording?.recordingURL {
            // If not saving, delete the temporary file
            do {
                try FileManager.default.removeItem(at: recordingURL)
                debugPrint("Temporary recording file deleted: \(recordingURL.lastPathComponent)")
            } catch {
                debugPrint("Failed to delete temporary recording file: \(error)")
            }
    ...
    func updateRecording(_ recording: AudioRecording) {
        ...
        if FileManager.default.fileExists(atPath: recording.recordingURL.path) {
            ...
            debugPrint("警告: 尝试更新不存在的录音文件: \(recording.recordingURL.path)")
        }
    }
    ...
    func saveRecording(recordingToSave: AudioRecording, forCharacterID characterID: UUID? = nil) {
        ...
        guard !recordingToSave.title.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
            debugPrint("Recording title cannot be empty.")
        ...
        guard FileManager.default.fileExists(atPath: recordingToSave.recordingURL.path) else {
            debugPrint("警告: 尝试保存不存在的录音文件: \(recordingToSave.recordingURL.path)")
            return
        }
    ...
    func deleteRecording(_ recording: AudioRecording) {
        ...
        if FileManager.default.fileExists(atPath: recording.recordingURL.path) {
            do {
                ...
                debugPrint("已删除音频文件: \(recording.recordingURL.path)")
            } catch {
                debugPrint("删除音频文件失败: \(error)")
            }
        } else {
            debugPrint("警告: 尝试删除不存在的录音文件: \(recording.recordingURL.path)")
        }
    ...
    func playRecording(recording: AudioRecording) {
        ...
        guard FileManager.default.fileExists(atPath: recording.recordingURL.path) else {
            debugPrint("警告: 尝试播放不存在的录音文件: \(recording.recordingURL.path)")
            return
        }
    ...
    func importAudioFile(from sourceURL: URL, withTitle title: String) async throws -> AudioRecording? {
        ...
        do {
            ...
            if sourceURL.startAccessingSecurityScopedResource() {
                ...
                debugPrint("尝试从 \(sourceURL.path) 导入音频文件")

                // 使用Data读取文件内容而不是直接复制文件
                do {
                    let audioData = try Data(contentsOf: sourceURL)
                    debugPrint("成功读取音频数据，大小: \(audioData.count) 字节")

                    // 将数据写入到目标文件
                    try audioData.write(to: destinationURL)
                    debugPrint("成功写入音频数据到: \(destinationURL.path)")
                    ...
                    debugPrint("读取或写入音频数据失败: \(dataError)")

                    // 尝试使用AVAsset复制音频数据
                    let audioAsset = AVURLAsset(url: sourceURL)
                    debugPrint("尝试使用AVAsset读取音频")
                                                                ...
                                                                presetName: AVAssetExportPresetAppleM4A)
                    {
                        ...
                        debugPrint("AVAsset导出失败")
                    }
                }
            } else {
                debugPrint("无法获取文件访问权限: \(sourceURL.path)")
            }

            return nil
        } catch {
            debugPrint("导入音频文件失败: \(error)")
            return nil
        }
    }
    ...
    func audioPlayerDidFinishPlaying(_ player: AVAudioPlayer, successfully flag: Bool) {
        ...
        //     try AVAudioSession.sharedInstance().setActive(false, options: .notifyOthersOnDeactivation)
        // } catch {
        //     debugPrint("Failed to deactivate audio session after playback: \(error)")
        // }
    }

    func audioPlayerDecodeErrorDidOccur(_ player: AVAudioPlayer, error: Error?) {
        debugPrint("Audio player decode error: \(String(describing: error))")
        isPlayingAudio = false
        currentlyPlayingAudioID = nil
    }
}
