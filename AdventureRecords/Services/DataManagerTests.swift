...
class DataManagerTests {
    ...
    func runAllTests() -> [TestResult] {
        ...
        do {
            ...
            results.append(TestResult(name: "备份创建测试", passed: false, message: "测试过程中发生异常: \(error.localizedDescription)"))
        }
        ...
        do {
            ...
            results.append(TestResult(name: "备份恢复测试", passed: false, message: "测试过程中发生异常: \(error.localizedDescription)"))
        }
        ...
        do {
            ...
            results.append(TestResult(name: "PDF导出测试", passed: false, message: "测试过程中发生异常: \(error.localizedDescription)"))
        }
        
        do {
            results.append(testDataExportText())
        } catch {
            results.append(TestResult(name: "文本导出测试", passed: false, message: "测试过程中发生异常: \(error.localizedDescription)"))
        }
        
        do {
            results.append(testDataExportJSON())
        } catch {
            results.append(TestResult(name: "JSON导出测试", passed: false, message: "测试过程中发生异常: \(error.localizedDescription)"))
        }
        ...
        do {
            ...
            results.append(TestResult(name: "清理所有数据测试", passed: false, message: "测试过程中发生异常: \(error.localizedDescription)"))
        }
        
        do {
            results.append(testDataCleanupCharacter())
        } catch {
            results.append(TestResult(name: "清理角色数据测试", passed: false, message: "测试过程中发生异常: \(error.localizedDescription)"))
        }
        
        do {
            results.append(testDataCleanupScene())
        } catch {
            results.append(TestResult(name: "清理场景数据测试", passed: false, message: "测试过程中发生异常: \(error.localizedDescription)"))
        }
        
        do {
            results.append(testDataCleanupNote())
        } catch {
            results.append(TestResult(name: "清理笔记数据测试", passed: false, message: "测试过程中发生异常: \(error.localizedDescription)"))
        }
            ...
            if exportDoc.data.count > 0 && exportDoc.filename.hasSuffix(".json") {
                ...
                do {
                    if let _ = try JSONSerialization.jsonObject(with: exportDoc.data, options: []) as? [String: Any] {
                        return TestResult(name: testName, passed: true, message: "成功导出JSON文档，大小: \(exportDoc.data.count) 字节")
                    } else {
                        return TestResult(name: testName, passed: false, message: "导出的JSON内容无效")
                    }
                } catch {
                    return TestResult(name: testName, passed: false, message: "无法解析导出的JSON: \(error.localizedDescription)")
                }
            } else {
